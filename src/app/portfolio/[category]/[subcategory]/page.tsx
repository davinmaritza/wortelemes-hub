...
export default function PortfolioSubcategoryPage({
  ...
  // DB stores subcategory as "parent/child" (e.g. "GTACommish/Vehicle")
  const dbCategory = `${category}/${subcategory}`;

  useEffect(() => {
    getPortfolioItems()
      .then((all) =>
        setItems(all.filter((item) => item.category === dbCategory)),
      )
      .catch(console.error)
      .finally(() => setIsLoading(false));
  }, [dbCategory]);

  const title = `${category.replace(/([A-Z])/g, " $1").trim()} – ${subcategory
    .replace(/([A-Z])/g, " $1")
    .trim()}`;
